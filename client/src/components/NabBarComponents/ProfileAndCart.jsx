    ...
    >
      ...
      <NavLink to={'/cart'} className="relative">
        <BiDish 
          className="h-10 w-10"
        />
        <span className="absolute bg-white text-black font-bold rounded-md w-6 h-6 text-center top-[-8px] right-[-16px]">{selector?.reduce((acc, cur)=> acc + cur.selectedQuantity, 0) || 0}</span>
      </NavLink>
    </div>
  );
};
